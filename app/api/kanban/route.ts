    ...
    `select e.chave, e.nome, e.cor,
            count(ct.id) filter (
              where $1::text is null
                 or ct.comprador_id in (select comprador_id from cs.contatos_ht where edicao = $1)
            )::int as total
       from cs.estagios e
  ...
  const cards = await query(
    `with base as (
       select h.comprador_id, h.nome, h.telefone, h.edicao, h.estagio_chave,
              ct.tags, ct.id as contato_id, h.ultima_resposta_em, ct.atualizado_em,
     ...
     select b.comprador_id, b.nome, b.telefone, b.edicao, b.estagio_chave, b.tags, b.ultima_resposta_em,
